...
export default class CustomInlineImage extends Plugin {
	init() {
		const editor = this.editor;
		const t = editor.t;

		editor.ui.componentFactory.add( 'customInlineImage', locale => {
			const view = new ButtonView( locale );

			view.set( {
				label: t( 'Insert inline image' ),
			...
			view.on( 'execute', () => {
				editor.fire( 'custom-inline-image', true );
			} );
